...
class GestureController:
    ...
    def __init__(self):
        ...
        self.alpha = 0.3  # smoothing factor for cursor
        self.last_click_time = 0
        self.active_mode = "None"
        self.mode_origin_y = None
        self.IDX_TIP, self.MID_TIP, self.RING_TIP, self.PINKY_TIP, self.THUMB_TIP = 8, 12, 16, 20, 4
        self.stable_counter = 0
        self.eye_typing_enabled = False  # NEW FLAG

    def _smooth_point(self, new_point):
        if self.smoothed_cursor is None:
            self.smoothed_cursor = np.array(new_point, dtype=float)
        else:
            self.smoothed_cursor = self.alpha * np.array(new_point) + (1 - self.alpha) * self.smoothed_cursor
        return tuple(self.smoothed_cursor.astype(int))
    ...
    def process_hand(self, frame):
        ...
        if not hand_results.multi_hand_landmarks:
            if self.active_mode == "Drag":
                pyautogui.mouseUp()
            self.active_mode = "None"
            return self.eye_typing_enabled  # return state
        ...
        sm_x, sm_y = self._smooth_point((ix, iy))
        ...
        # --- NEW GESTURES ---
        # Start Typing: Thumb + Index close
        d_thumb_index = np.hypot(
            pts[self.THUMB_TIP][0] - pts[self.IDX_TIP][0],
            pts[self.THUMB_TIP][1] - pts[self.IDX_TIP][1]
        )
        if d_thumb_index < 50:  # threshold for pinch
            self.eye_typing_enabled = True
            self.active_mode = "StartTyping"
            return self.eye_typing_enabled

        # Stop Typing: Pinky + Thumb close
        if d_pinky_thumb < 50:
            self.eye_typing_enabled = False
            self.active_mode = "StopTyping"
            return self.eye_typing_enabled

        # --- If typing is active: skip hand gestures ---
        if self.eye_typing_enabled:
            return self.eye_typing_enabled
        ...
        if d_idx_mid < 50:
            ...
            if self.stable_counter > 3 and now - self.last_click_time > Config.ACTION_COOLDOWN_S:
                pyautogui.click()
        ...
        if d_mid_ring < 50 and now - self.last_click_time > Config.ACTION_COOLDOWN_S:
        ...
        elif self.active_mode == "Drag":
            pyautogui.mouseUp()
            self.active_mode = "None"

        elif d_ring_thumb < 50:
            self.active_mode = "Brightness"
            if self.mode_origin_y is None:
                self.mode_origin_y = pts[self.RING_TIP][1]
            delta = self.mode_origin_y - pts[self.RING_TIP][1]
            if abs(delta) > 70:
                self._adjust_system_control("Brightness", delta)
                self.mode_origin_y = pts[self.RING_TIP][1]

        elif d_idx_mid > 180:
            self.active_mode = "Scroll"
            if self.mode_origin_y is None:
                self.mode_origin_y = iy
            pyautogui.scroll(int(-(iy - self.mode_origin_y) / Config.SCROLL_SENSITIVITY))

        if self.active_mode not in ["Volume", "Brightness", "Scroll"]:
            self.mode_origin_y = None
...
class UI:
    ...
    def draw_keyboard(self, canvas, gaze_pt, blink_occurred, typing_enabled):
        if not typing_enabled:
            return  # hide keyboard when typing is off
        for r, row in enumerate(self.keys):
            row_width = len(row) * self.key_size[0]
            start_x = (self.width - row_width) // 2
            if row[0] == 'SPACE': 
                start_x = (self.width - self.key_size[0] * len(self.keys[1])) // 2
            for c, key in enumerate(row):
                key_w = self.key_size[0]
                if key == 'SPACE':
                    key_w = len(self.keys[1]) * self.key_size[0]
    ...
    def draw_main_canvas(self, cam_frame, gaze_pt, gesture_mode, eye_tracker, typing_enabled):
        ...
        self.draw_keyboard(canvas, gaze_pt, eye_tracker.blink_registered, typing_enabled)
        if typing_enabled and gaze_pt:
    ...
    def create_gesture_guide_image(self):
        ...
        gestures = [
            ("Start Typing:", "Index+Middle+Thumb"),
            ("Stop Typing:", "Pinky+Thumb"),
...
def main():
    ...
    while True:
        ...
        typing_enabled = gesture_controller.process_hand(rgb_frame)

        small_cam = cv2.resize(frame, (240, 180))
        canvas = ui.draw_main_canvas(small_cam, gaze_pt, gesture_controller.active_mode, eye_tracker, typing_enabled)
